...
//date_default_timezone_set($timezone);
	...
	$Message_Data=mysql_query("INSERT INTO message(developer_id,client_id,project_id,Sender_Id,Message,ReadStatus,Date) values('".$developer_id."','".$client_id."','".$project_id."','".$developer_id."','".$Message."','1','".showcurrenttime()."')");
	...
	$client_data=mysql_fetch_array(mysql_query("select * from register where unique_code='".$client_id."'"));
		...
		$subject  = $client_data['first_name'].' '.$client_data['last_name'].", you have received a message from ".$developer_data['first_name'].' '.$developer_data['last_name'];
		$header   = "Webzira.com"; 
		$msg='
			...
			<div bgcolor="#e5e5e1" style="margin:0;padding:0">
				<table width="100%" bgcolor="#e5e5e1" cellspacing="0" cellpadding="0">
					<tbody>
						...
						Hi,'.$client_data['first_name']." ".$client_data['last_name'].'!
										</td>
									</tr>
									<tr>
						<td style="font-family:Arial,Helvetica,sans-serif;font-size:16px;color:#000;line-height:24px;word-spacing:1px;">User Name : \''.$client_data['username'].'\'</td>
						</tr>
									<tr>
					   ...
					   You have received new message from <strong>'.$developer_data['first_name'].' '.$developer_data['last_name'].'</strong>. 
									
						<br /><br>
						
						<a style="background:#41b7d8 none repeat scroll 0 0; color:white; padding:10px 20px; text-decoration:none; font-family: Source Sans Pro,sans-serif; font-size:20px;" href="'. WebUrl.'Client/ClientMessage.php?project_id='.$project_id.'&code='.$developer_data['unique_code'].'" >View Message</a>
