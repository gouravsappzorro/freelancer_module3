...
$(window).load(function(){
	...
	//setTimeout(function() {window.location.href = $('.pay').attr('href');}, 50);
});
</script>
</head>
...
// check if API caller is the money sender (implicit payment)
		if('buyer@greencubes.co.in' == $_POST["senderEmail"]) {
			$case ="3";
		} else {
			$case ="2";
		}
	...
	switch($case) {
		...
		case "2" :
			...
			echo "<tr><td>Ack :</td><td><div id='Ack'>$ack</div> </td></tr>";
			echo "<tr><td>PayKey :</td><td><div id='PayKey'>$payKey</div> </td></tr>";
			echo "<tr><td><a href=$payPalURL class='pay'><b>Redirect URL to Complete Payment </b></a></td></tr>";
			echo "</table>";
			?>
			<script>
				$(window).load(function() {
				$(".se-pre-con").fadeIn("fast");
    			setTimeout(function() {window.location.href = $('.pay').attr('href');}, 1); });
			</script>
<?php
			break;
		case "3" :
