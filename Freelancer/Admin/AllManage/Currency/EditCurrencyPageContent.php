	...
	<div id="container">
		...
		<div id="content">
			<div class="container">
				...
				$Edit = mysql_fetch_array(mysql_query("SELECT * FROM admin_currency WHERE CodeId = '$Code'"));
						if(isset($_POST['NewPageAdd']))
						{
						   if($_POST["CurrencyName"] == "")
							{
								echo '<div class="alert alert-danger fade in">
										<i class="icon-remove close" data-dismiss="alert"></i>
										<strong>Please Enter Any Currency.....</strong> .
									</div>';
							}
							else
							{
							...
							$CodeId           = rand(100000, 999999);
								...
								$select =mysql_query("SELECT * FROM `admin_currency` where Currency='$CurrencyName'");
							$row=mysql_fetch_array($select);
							if(!$row["Currency"])
							{				
							$Update           =	"UPDATE admin_currency SET
							Status            =  '$Status',
					...
					WHERE	CodeId            =  '$Code' ";	
							...
							echo '<div class="alert alert-danger fade in">
										<i class="icon-remove close" data-dismiss="alert"></i>
										<strong>Currency Name Allready Exist.....</strong> .
									</div>';
				...
				<form name="individual_signup" class="form-horizontal row-border"  action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return JayCreatedValidForm(this);" method="post"  enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<label class="col-md-2 control-label">Currency Name<span class="required">*</span></label>
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
										
																<div class="input-group">
																	<span class="input-group-addon"><i class="icon-cog"></i></span>
																		<input value="<?php echo $Edit['Currency'];?>" type="text" name="CurrencyName" id="Name"  title="CurrencyName" onFocus="if (this.value == 'CurrencyName') {this.value = '';}" onBlur="CurrencyNameCheck(this.value)" class="form-control bs-tooltip" >
																	</div>
										</div>
