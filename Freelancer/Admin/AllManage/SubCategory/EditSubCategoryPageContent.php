...
<head>
	...
	<title>Admin Panel - Edit Sub Category</title>
	...
	<div id="container">
		...
		<div id="content">
			<div class="container">
				...
				$Edit = mysql_fetch_array(mysql_query("SELECT * FROM admin_subcategory WHERE CodeId = '$Code'"));
						if(isset($_POST['NewPageAdd']))
						{
						  
							if($_POST["CategoryName"] == "" || $_POST["SubCategoryName"] == "")
							{
							...
							$select=mysql_query("SELECT * FROM `admin_subcategory` where SubCategory='$SubCategoryName' and CodeId!='$Code'");
							$row=mysql_fetch_array($select);
							if($row["SubCategory"]!=$SubCategoryName)
							{
										
								$Update           =	"UPDATE admin_subcategory SET
								Status            =  '$Status',
						...
						WHERE	CodeId            =  '$Code' ";	
							...
							echo '<div class="alert alert-danger fade in">
										<i class="icon-remove close" data-dismiss="alert"></i>
										<strong>Sub Category Name Allready Exist.....</strong> .
									</div>';
										...
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
																...
																<div class="input-group">
                            										...
                            										<?php 
																	$Code1=$_GET["code"];
																	$query1=mysql_query("SELECT  * FROM admin_subcategory where Category_Id='$Code1'");
																	$row1=mysql_fetch_array($query1);
																	 $query=mysql_query("SELECT  * FROM admin_category");
																	while($row=mysql_fetch_array($query)){ ?>
	<option title="CategoryName" onFocus="if (this.value == "CategoryName") {this.value = "";}" onBlur="CategoryNameCheck(this.value)" class="form-control bs-tooltip" value='<?php echo $row['Id']; ?>' <?php if($row['Id']==$row1['Category_Id']) echo 'selected="selected"'; ?>><?php echo $row['CategoryName']; ?></option><?php 
                                                                    } ?>
                                                                    </select>
									...
									<div class="form-group">
										...
										<label class="col-md-2 control-label">SubCategoryName<span class="required">*</span></label>
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
										
																<div class="input-group">
																	<span class="input-group-addon"><i class="icon-cog"></i></span>
																		<input value="<?php echo $Edit['SubCategory']; ?>" type="text" name="SubCategoryName" id="Name"  title="SubCategoryName" onFocus="if (this.value == 'SubCategoryName') {this.value = '';}" onBlur="SubCategoryNameCheck(this.value)" class="form-control bs-tooltip" >
																	</div>
										</div>
