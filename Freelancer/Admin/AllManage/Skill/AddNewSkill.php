					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> ADD NEW SKILL</h4>
						...
						if(isset($_POST['NewPageAdd']))
						{
						  if($_POST["SkillName"] == "")
						  {
							  echo '<div class="alert alert-danger fade in">
										...
										<strong>Please Enter Any Skill</strong> .
									</div>';
						  }
						  else
						  {
							...
							$select =mysql_query("SELECT * FROM `admin_skill` where Skill_Name='$SkillName'");
							$row=mysql_fetch_array($select);
							if(!$row["Skill_Name"])
							{
								$Insert           =	"INSERT INTO admin_skill SET
							CodeId            = '$CodeId',
							Status			='$Status',
							type='$CategoryName',
				...
				<form name="individual_signup" class="form-horizontal row-border"  action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return JayCreatedValidForm(this);" method="post"  enctype="multipart/form-data">
                        	...
                        	<div class="form-group">
										...
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
																...
																<div class="input-group">
                                                                    ...
                                                                    <select name="CategoryName" id="CategoryName" class="form-control bs-tooltip">
                                                                    <option value="">Select Category</option>
                            										<?php 
																	 $query=mysql_query("SELECT  * FROM admin_skill_category");
																	while($row=mysql_fetch_array($query)){
																	?>
                                                                    <option title="CategoryName" onFocus="if (this.value == 'CategoryName') {this.value = '';}" onBlur="CategoryNameCheck(this.value)" class="form-control bs-tooltip" value="<?php echo $row['CodeId']; ?>"><?php echo $row['skill_category']; ?></option>
                                                                    <?php } ?>
                                                                    </select>
									...
									<div class="form-group">
										...
										<label class="col-md-2 control-label">Skill Name<span class="required">*</span></label>
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
										
																<div class="input-group">
																	<span class="input-group-addon"><i class="icon-cog"></i></span>
																		<input type="text" name="SkillName" id="Name"  title="SkillName" onFocus="if (this.value == 'SkillName') {this.value = '';}" onBlur="SkillNameCheck(this.value)" class="form-control bs-tooltip" >
																	</div>
										</div>
