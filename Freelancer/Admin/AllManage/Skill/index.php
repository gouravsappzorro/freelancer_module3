...
<head>
	...
	<title>Admin Panel - Manage Skills</title>

	<?php include "../../MyInclude/HomeScript.php"; ?>
	<script src="<?php echo AdminUrl;?>plugins/datatables/jquery.dataTables.js"></script>
	<script src="<?php echo AdminUrl;?>plugins/datatables/jquery.dataTables.min.js"></script>
	<script src="<?php echo AdminUrl;?>plugins/datatables/DT_bootstrap.js"></script>
    <link href="<?php echo AdminUrl;?>plugins/datatables/dataTables.responsive.css" type="text/css" rel="stylesheet">
    <link href="<?php echo AdminUrl;?>plugins/datatables/dataTables.bootstrap.css" type="text/css" rel="stylesheet">
...
<body>
	...
	<?php 
				 if(isset($_GET['AJCOde59']) and $_GET['AJCOde59'] !=='')
					   {
					   			$Delete = mysql_query("DELETE FROM admin_skill WHERE CodeId = '".$_GET['AJCOde59']."' ");
								if($Delete)
								{
				 ...
				 if(isset($_GET['DEAJ09148']) and $_GET['DEAJ09148'] !=='')
					   {
					   			$UPDATW = mysql_query("UPDATE admin_skill SET Status = 'Published' WHERE CodeId = '".$_GET['DEAJ09148']."' ");
								if($UPDATW)
								{
				 ...
				 if(isset($_GET['ActAJ09148']) and $_GET['ActAJ09148'] !=='')
					   {
					   			$UPDATW = mysql_query("UPDATE admin_skill SET Status = 'Pending' WHERE CodeId = '".$_GET['ActAJ09148']."' ");
								if($UPDATW)
								{
				...
				<?php 	if(isset($_SESSION['UpdateSu']) and $_SESSION['UpdateSu'] == 'Done' )
					   ...
					   if(isset($_SESSION['SETAcSu']) and $_SESSION['SETAcSu'] = 'Pending')
						...
						if(isset($_POST['Delete']))
						{
						  if(isset($_POST['Code'])) { $checkbox   =   $_POST['Code']; } else { $checkbox  = '';} 
						   ...
						   {
							 $del_id   =  $checkbox[$i];
							 $sql      = "DELETE FROM admin_skill WHERE CodeId='$del_id'";
				 ...
				 ?>		
                  <p style="text-align:right;"><a href="AddNewSkill.php"><input type="button" name="new" class="btn btn-primary pull-center" value="AddNewSkill"></a></p>		   
                  	<form name="individual_signup" action="" method="post" >
						<div class="widget box table-responsive">
							<div class="widget-header">
						
								<h4><i class="icon-reorder"></i> View All Skill</h4>
							...
							<div class="widget-content no-padding">
								...
								<table class="table table-striped  table-hover table-checkable table-colvis datatable dataTable">
									...
									<?php $i=0;
					 				$res = mysql_query("SELECT  * FROM admin_skill ORDER BY Id DESC");
						  
									while ($Fw = mysql_fetch_array($res)) 
									{	 
									$id=$Fw["type"];
									$query=mysql_query("SELECT  * FROM admin_skill_category where CodeId = '$id' ORDER BY Id DESC ");
									...
									?>
                                        ...
                                        <?php  if($Fw['Status'] == 'Pending') 
                                                { ?>
                                                        <a href="index.php?DEAJ09148=<?php echo $Fw['CodeId']; ?>"  class="btn btn-sm bs-tooltip" title="Make it Active" >
                                                        <i class="icon-fixed-width">&#xf00d;</i> Deactive
                                                        </a>
                                           <?php } ?>
                                        <?php   if($Fw['Status'] == 'Published') 
                                                  {  ?>	
                                                         <a href="index.php?ActAJ09148=<?php echo $Fw['CodeId']; ?>" class="btn btn-sm bs-tooltip" title="Make it Deactive" >
                                                         <i class="icon-fixed-width">&#xf00c;</i> Active
                                                         </a>                
                                            <?php } ?>
                                                          <a href="./EditSkillPageContent.php?AJCOde59=<?php echo $Fw['CodeId']; ?>&code=<?php echo $row['CodeId']; ?>" class="btn btn-sm bs-tooltip" title="Edit">
