	...
	<div id="container">
		...
		<div id="content">
			<div class="container">
				...
				$Edit = mysql_fetch_array(mysql_query("SELECT * FROM admin_category WHERE CodeId = '$Code'"));
						if(isset($_POST['NewPageAdd']))
						{
							...
							else
							{
									...
									$select =mysql_query("SELECT * FROM `admin_category` where CategoryName='$CategoryName' and CodeId != '$Code'");
									$row=mysql_fetch_array($select);
									if(!$row["CategoryName"])
									{
														
											$Update           =	"UPDATE admin_category SET
											Status            =  '$Status',
							...
							if($UPData)
							{
								...
								echo  '<div class="alert alert-success fade in">
													<i class="icon-remove close" data-dismiss="alert"></i>
													<strong>Your Category Update Successfully!</strong>.
									   </div>';
							...
							echo '<div class="alert alert-danger fade in">
										<i class="icon-remove close" data-dismiss="alert"></i>
										<strong>Category Name Allready Exist.....</strong> .
									</div>';
				...
				<form name="individual_signup" class="form-horizontal row-border"  action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return JayCreatedValidForm(this);" method="post"  enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<label class="col-md-2 control-label">CategoryName<span class="required">*</span></label>
										<div class="col-md-9">
															<label for="Name" class="error" id="Name_error"></label>
										
																<div class="input-group">
																	<span class="input-group-addon"><i class="icon-cog"></i></span>
																		<input type="text" name="CategoryName" value="<?php echo $Edit['CategoryName']; ?>"  id="Name"  title="CategoryName" onFocus="if (this.value == 'CategoryName') {this.value = '';}" onBlur="CategoryNameCheck(this.value)" class="form-control bs-tooltip" >
																	</div>
										</div>
