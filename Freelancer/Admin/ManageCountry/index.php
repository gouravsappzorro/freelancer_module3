...
<body>
	...
	<?php 
				 if(isset($_GET['AJCOde59']) and $_GET['AJCOde59'] !='')
					   {
								...
								$Delete=mysql_query("DELETE FROM location WHERE Id = '".$_GET['AJCOde59']."' ");
				 ...
				 if(isset($_GET['DEAJ09148']) and $_GET['DEAJ09148'] !='')
					   {
					   			$UPDATW = mysql_query("UPDATE location SET Status = 'Published' WHERE Id = '".$_GET['DEAJ09148']."' ");
								if($UPDATW)
								{
				 ...
				 if(isset($_GET['ActAJ09148']) and $_GET['ActAJ09148'] !='')
					   {
					   			$UPDATW = mysql_query("UPDATE location SET Status = 'Pending' WHERE Id = '".$_GET['ActAJ09148']."' ");
								if($UPDATW)
								{
				...
				<div class="row">
					...
					<div class="col-md-12">
						...
						/*if(isset($_SESSION['UpdateSu']) and $_SESSION['UpdateSu'] == 'Done' )
						{ 
					   ...
					   if(isset($_SESSION['SETAcSu']) and $_SESSION['SETAcSu'] == 'Published' )
						{ 
							echo	'<div class="alert alert-success fade in">
											<i class="icon-remove close" data-dismiss="alert"></i>
											<strong>Country Activated Successfully!</strong>.
									  </div>';
					   ...
					   if(isset($_SESSION['SETAcSu']) and $_SESSION['SETAcSu'] == 'Pending')
						{ 
							echo	'<div class="alert alert-success fade in">
											<i class="icon-remove close" data-dismiss="alert"></i>
											<strong>Country Deactivated Successfully!</strong>.
									  </div>';
						    unset($_SESSION['SETAcSu']);
			           }
				 ...
				 ?>	
                 <p style="text-align:right;"><a href="AddNewCountry.php"><input type="button" name="new" class="btn btn-primary pull-center" value="AddNewCountry"></a></p>
                       			<form name="individual_signup" action="" method="post" >
						<div class="widget box table-responsive">
							<div class="widget-header">
						
								<h4><i class="icon-reorder"></i> View All Countries</h4>
							...
							<div class="widget-content no-padding">
								...
								<table class="table table-striped table-hover table-checkable table-colvis datatable dataTable">
                                    ...
                                    $res = mysql_query("SELECT  * FROM location ORDER BY Name");
									...
									<?php  if($Fw['Status'] == 'Pending') 
											{ ?>
													<a href="index.php?DEAJ09148=<?php echo $Fw['Id']; ?>"  title="Make it Active" class="btn btn-sm bs-tooltip">
													<i class="icon-fixed-width">&#xf00d; </i> Deactive
													</a>
									   <?php } ?>
									<?php   if($Fw['Status'] == 'Published') 
											  {  ?>	
													 <a href="index.php?ActAJ09148=<?php echo $Fw['Id']; ?>" title="Make it Deactive" class="btn btn-sm bs-tooltip">
													 <i class="icon-fixed-width">&#xf00c; </i> Active
													 </a>                
										<?php } ?>
													  <a href="./EditCountry.php?AJCOde59=<?php echo $Fw['Id']; ?>" title="Edit" class="btn btn-sm bs-tooltip">
													  <i class="icon-pencil"></i> Edit
													  </a>
													  
													   <a  href="index.php?AJCOde59=<?php echo $Fw['Id']; ?>" onClick="return confirm('are you sure you want to delete??');" title="Delete" class="btn btn-sm bs-tooltip">
