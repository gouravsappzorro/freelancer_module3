...
'.$row['image'].'</a>';
                        ...
                        <br />
                        <h3 class="portfolio-single-heading title style1 bw-2px dh-2px divider-primary bc-dark dw-default color-default" style="margin-bottom:30px;"><span>Add
                         Question & Answers</span></h3>
                         <?php
                         if(isset($_POST['editfaqbutton']))
                         {
                         	extract($_POST);
                         	$update_faq=mysql_query("UPDATE faqs set Question='".$editquestion."',Answer='".$editanswer."' where UniqueId='".$updatefaqid."'");
                         }
                         if(@$update_faq)
                         {
                         	echo '<div class="alert alert-success alert-dismissible">
								    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								    <strong>Success!</strong> Your Question & Answer Update Successfully.
								  </div>';
                         }
                         ?>
                         <div id="refreshdiv">
                         <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal" id="clickbutton" style="display: none;"></button>
                         <div id="editmodal"></div>
                        <div class="excerpt">
                          <div class="panel-group" id="accordion">
                        <?php
                        $i=1;
                        $select_faq=mysql_query("SELECT * FROM `faqs` where ProjectId='".$_GET['project_id']."' and ClientId='".$_SESSION['id']."'");
                        ...
                        ?>
						    ...
						    <div class="panel panel-warning">
						      ...
						      <div id="collapse<?php echo $i;?>" class="panel-collapse collapse">
			                    ...
			                    &nbsp;&nbsp;<a href="javascript:void(0)" id="<?php echo $fetch_faq['UniqueId'];?>" onclick="editfaq(this.id)" class="fa fa-pencil fa-1x" data-toggle="tooltip" data-placement="right" title="" data-original-title="FA-PENCIL"><i></i></a>&nbsp;
			                    <a href="javascript:void(0)" id="<?php echo $fetch_faq['UniqueId'];?>" onclick="removefaq(this.id)" class="fa fa-times-circle fa-1x" data-toggle="tooltip" data-placement="right" title="" data-original-title="FA-SEARCH"><i></i></a>
							</div><!-- /FOOTER PANEL--> 
						      </div>
                        ...
                        {
                        	extract($_POST);
                        	$UniqueCode=md5(date('d-m-Y H:m:s'));
                        	$insert_faq=mysql_query("INSERT INTO `faqs` SET UniqueId='".$UniqueCode."',
                        ...
                        if(@$insert_faq)
                        {
                        	echo '<div class="alert alert-success alert-dismissible">
								    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								    <strong>Success!</strong> Your Question & Answer Add Successfully.
								  </div>';
						    ?>
						    <script type="text/javascript">
						    	setTimeout(function() {
                        ...
                        ?>
                            ...
                            <div>
                           <input type="text" name="question" style="width: 80%;" placeholder="Enter Your Question " required="">
                           <br />
							<textarea name="answer" style="width: 80%;" placeholder="Write Your Answer" required=""></textarea>
							<br />
							</div>
							</div>
							<input type="submit" class="external" name="addfaq" value="Submit">
                            <form>
                        </div>
                        </div>
                    </div>
	...
	?>
        ...
        <table class="table table-hover">
            ...
            while($mlstn=mysql_fetch_array($sql_mlstn))
            {
				...
				echo '<td>'.$mlstn['create_date'].'</td>';
				if($mlstn['release_date']=='0000-00-00')
				{
					echo '<td></td>';
				}
				else{
					echo '<td>'.$mlstn['release_date'].'</td>';
				}
  ...
  //-------------------------For More Phone Number-----------------------
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
             //text box increment
            $(wrapper).append("<div><a href='#' class='remove_mobile_field col-md-10'><strong>- Remove</strong></a><input type='text' name='question[]' class='form-control' style='width: 60%;' placeholder='Enter Your Question '><br /><textarea name='answer' style='width: 70%;' placeholder='Write Your Answer'></textarea><br /></div>"); 
                                                   //add input box
        x++;
    }
    });
    ...
    $(wrapper).on("click",".remove_mobile_field", function(e){ //user click on remove text
      ...
      function editfaq(editid){
      	// alert(editid);
      	$.ajax({
      		url:'EditFaqModal.php',
      		type:'POST',
      		data:'editid='+editid,
      		success:function(updatefaq){
      			// alert(updatefaq);
      			$('#editmodal').html(updatefaq);
