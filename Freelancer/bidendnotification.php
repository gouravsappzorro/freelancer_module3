...
<?php
$sql_projects=mysql_query("select  * from post_projects where uid='".$_SESSION['id']."'");
while($projects=mysql_fetch_array($sql_projects))
{
	$date = $projects['bid_end_date'];
	$day_before = date( 'Y-m-d', strtotime( $date . ' -3 days' ) );
	
	$current_date=date('Y-m-d');
	//echo $DATE='2016-05-25'.'<br>';
	
	//if($DATE==$current_date)
	if($day_before==$current_date)
	{
		$to       = $row['email'];
		$subject  = "Bid Ends Notification on ".$projects['title'];
		$header   = "Webzira.com"; 
		$msg='
						...
						Hi,'.$row['first_name']." ".$row['last_name'].'!
									...
									<tr>
										...
										<span class="il">Welcome</span> to Webzira !<br><br>
					   					...
					   					Your Posted project <strong>'.$projects['title'].'</strong> is expire after 3 days
...
?>
<!--=========End Project Complete Notification =========-->